...
async function check() {
  ...
  const testCols = ['status', 'submitter_id', 'project_id', 'description', 'history', 'source', 'assignee_id', 'organization_id'];
  for (const c of testCols) {
    const p = { ...payload, [c]: 'test' };
    const { error } = await supabase.from('requirements').insert(p);
