...
code = code.replace(
  ...
  \`<section className={\\\`\${currentView === 'tracking' ? 'lg:col-span-12 p-0' : 'lg:col-span-9 p-0 sm:p-6 lg:p-8 pt-4 sm:pt-6'} flex flex-col sm:border-b border-[#1A1A1A]/10 lg:border-b-0 h-[100dvh] lg:h-full overflow-y-auto lg:overflow-y-auto bg-[#F7F6F2] \${currentView === 'dashboard' ? 'order-1 lg:order-2 pb-32 sm:pb-6' : 'pb-24 sm:pb-6'}\\\`}>\`
);

// We need to also modernize the charts layer
let chartsRegex = /<div className="grid grid-cols-1 lg:grid-cols-2 gap-10">[\\s\\S]*?<\\/div>\\s*<\\/div>\\s*\\)\\}\\s*<\\/div>\\s*\\)\\}\\s*<\\/section>/;

// A simpler way:
// code = code.replace(/className="bg-white border border-\\[#1A1A1A\\]\\/10 p-6 shadow-sm hover:shadow-md transition-shadow"/g, 'className="bg-white/80 backdrop-blur-md border border-black/5 p-6 rounded-[24px] sm:rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)]"');
// code = code.replace(/className="bg-white border border-\\[#1A1A1A\\]\\/10 p-6 lg:col-span-2 shadow-sm hover:shadow-md transition-shadow"/g, 'className="bg-white/80 backdrop-blur-md border border-black/5 p-6 lg:col-span-2 rounded-[24px] sm:rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)]"');
